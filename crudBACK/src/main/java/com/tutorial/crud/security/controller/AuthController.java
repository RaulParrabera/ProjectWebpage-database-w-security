...
public class AuthController {
    ...
    public ResponseEntity<?> nuevo(@Valid @RequestBody NuevoUsuario nuevoUsuario, BindingResult bindingResult){
        ...
        if(usuarioService.existsByNombreUsuario(nuevoUsuario.getNombreUsuario()))
            return new ResponseEntity(new Mensaje("Nombre inexistente o no válido"),HttpStatus.BAD_REQUEST);
        if (usuarioService.existsByEmail(nuevoUsuario.getEmail()))
            return new ResponseEntity(new Mensaje("Email inexistente o no válido"),HttpStatus.BAD_REQUEST);
        Usuario usuario =
        ...
        if (nuevoUsuario.getRoles().equals("admin"))
            // TODO: 03/12/2021 cambiar condición la comparación de contains
